...
namespace Universe
{
	CCyberNode::CCyberNode(const wchar_t* pName, u32 sceneHash) :
	...
	void CCyberNode::Initialize()
	{
		...
		{ // Create the mesh renderer.
			...
			data.pMaterial = m_pMaterial = reinterpret_cast<Graphics::CMaterial*>(Resources::CManager::Instance().GetResource(Resources::RESOURCE_TYPE_MATERIAL, Math::FNV1a_64("MATERIAL_CYBER_NODE")));
	...
	void CCyberNode::InteractCallback(void* pVal)
	{
		...
		if(pInfo->val == 2)
		{
			App::CSceneManager::Instance().LoadScene(App::SCENE_HASH_NODE);
		}
	}
};
