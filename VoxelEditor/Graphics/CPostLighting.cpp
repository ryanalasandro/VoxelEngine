...
#include <Graphics/CRenderTexture.h>
...
namespace Graphics
{
	...
	const CRenderTexture** CPostLighting::ApplyEffect(const CRenderTexture** ppTargetTextureList, u32 targetTextureCount,
		const CRenderTexture** ppPostTextureList, u32 postTextureCount, u32& outRenderTextureCount)
	{
		...
		m_pPostMat->SetTexture(edgeHash, m_data.m_pPostEdge->GetRenderTextureList()[0]->GetRTVTexture(0));
		m_pPostMat->SetTexture(ssaoHash, m_data.m_pPostSSAO->GetRenderTextureList()[0]->GetRTVTexture(0));
		m_pPostMat->SetTexture(skyHash, ppPostTextureList[0]->GetRTVTexture(0));
	...
	void CPostLighting::OnResize(const App::CPanel* pPanel)
	{
		for(u32 i = 0; i < m_renderTextureCount; ++i)
		{
			m_pRenderTextureDataList[i].width = static_cast<u32>(m_pGraphicsAPI->GetWidth());
