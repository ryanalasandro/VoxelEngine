...
namespace Math
{
	...
	{
		return Matrix3x3(rows[0] + m.rows[0], rows[1] + m.rows[1], rows[2] + m.rows[2]);
	}

	inline Matrix3x3 Matrix3x3::operator + (float f) const
	{
		return Matrix3x3(rows[0] + f, rows[1] + f, rows[2] + f);
	}
	...
	// This only works for real symmetric matrices.
	void Matrix3x3::EigenDecomposition(Math::Vector3& eigenvalues, Matrix3x3& eigenvectors) const
	{
		std::array<float, 3> eval {};
		std::array<std::array<float, 3>, 3> evec {};
		NISymmetricEigensolver3x3<float>()(rows[0][0], rows[0][1], rows[0][2], rows[1][1], rows[1][2], rows[2][2], 0, eval, evec);

		eigenvalues = Vector3(eval[0], eval[1], eval[2]);
		eigenvectors = Matrix3x3(evec[0][0], evec[0][1], evec[0][2], evec[1][0], evec[1][1], evec[1][2], evec[2][0], evec[2][1], evec[2][2]);

		// Calculate eigenvalues.
		/*const float p1 = (rows[0][1] * rows[0][1]) + (rows[0][2] * rows[0][2]) + (rows[1][2] * rows[1][2]);
		if(p1 == 0.0f) {
			eigenvalues[0] = rows[0][0];
			eigenvalues[1] = rows[1][1];
			eigenvalues[2] = rows[2][2];
		} else {
			const float q = Trace() * g_1Over3;

			const float p2 = powf(rows[0][0] - q, 2) + powf(rows[1][1] - q, 2) + powf(rows[2][2] - q, 2) + p1 * 2.0f;
			const float p = sqrtf(p2 * g_1Over6);
			const Matrix3x3 B = (*this - MTX3X3_IDENTITY * q) * (1.0f / p);
			const float r = B.Determinate() * 0.5f;
			...
			if(r <= -1.0f) {
				...
				phi = acosf(r) * g_1Over3;
			}

			eigenvalues[0] = q + p * cosf(phi) * 2.0f;
			eigenvalues[2] = q + p * cosf(phi + (g_2Pi * g_1Over3)) * 2.0f;
			eigenvalues[1] = q * 3.0f - eigenvalues[0] - eigenvalues[2];
		}*/

		// Calculate eigenvectors.
	}

	Matrix3x3 Matrix3x3::PseudoInverse() const
	{
		Math::Vector3 eval;
		Math::Matrix3x3 evec;
		...
		Math::Matrix3x3 sigma(
			Reciprocal(eval[0]), 0.0f, 0.0f,
			0.0f, Reciprocal(eval[1]), 0.0f,
			0.0f, 0.0f, Reciprocal(eval[2])
		);

		return Math::Matrix3x3::Transpose(evec) * sigma * evec;
	}
