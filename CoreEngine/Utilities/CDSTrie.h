...
namespace Util
{
	...
	public:
		...
		void AddSequence(const std::string& str, const T& value)
		{
			assert(str.length() > 0);
		...
		Node* Step(char ch, Node* pNode) const
		{
		...
		Node* Insert(char ch, Node* pNode)
		{
			Node* pNewNode = new Node();
			pNewNode->prefix = ch;
		...
		bool Find(const std::string& str, T& output) const
		{
			Node* pNode = m_pHead;
			for(int i = 0; i < str.length(); ++i)
			{
