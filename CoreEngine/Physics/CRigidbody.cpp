...
namespace Physics
{
	...
	bool CRigidbody::Response()
	{
		...
		if(m_bLastHit)
		{
			if(m_contactList.size() > 1)
			{
				OutputDebugStringA(std::to_string(m_contactList.size()).c_str());
				OutputDebugStringA("\n");
			}
			...
			// eFrom, Radius, eVelocity, m_pIntersections, IntersectionCount
	...
	{
		m_bOnGround |= _mm_cvtss_f32(Math::SIMDVector::Dot(normal, Math::SIMD_VEC_UP)) > 0.25f;

		m_solverPosition += contact;
		if(_mm_cvtss_f32(Math::SIMDVector::Dot(normal, m_solverVelocity)) < 0.0f)
		{
			m_solverVelocity -= normal * _mm_cvtss_f32(normal.Dot(m_solverVelocity));
		}
