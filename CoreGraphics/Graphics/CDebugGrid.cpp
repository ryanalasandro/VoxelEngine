...
namespace Graphics
{
	CDebugGrid::CDebugGrid(const wchar_t* pName, u32 sceneHash) :
	...
	void CDebugGrid::Initialize()
	{
		{ // Create the mesh data.
			...
			{
				*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.right * (-wh + m_data.cellSize.x * i) + m_data.forward * -lh };
				*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.right * (-wh + m_data.cellSize.x * i) + m_data.forward * lh };
			}

			for(u32 i = 0; i < m_data.length; ++i)
			{
				*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.right * -wh + m_data.forward * (-lh + m_data.cellSize.y * i) };
				*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.right * wh + m_data.forward * (-lh + m_data.cellSize.y * i) };
			}
		}
