...
namespace Graphics
{
	CDebugOBB::CDebugOBB(const wchar_t* pName, u32 sceneHash) :
	...
	void CDebugOBB::Initialize()
	{
		{ // Create the mesh data.
			...
			// Bottom.
			*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.offset + Math::Vector3(-m_data.halfSize.x, -m_data.halfSize.y, -m_data.halfSize.z) };
			*(Vertex*)m_meshData.GetVertexAt(index++) = { m_data.offset + Math::Vector3( m_data.halfSize.x, -m_data.halfSize.y, -m_data.halfSize.z) };
