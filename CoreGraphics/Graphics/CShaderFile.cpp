...
namespace Graphics
{
	...
	{
		std::ifstream input(m_data.filename, std::ios::ate);
		assert(input.is_open());
		const size_t sz = input.tellg();
		input.seekg(0, std::ios::beg);

		std::string fullFile;
		fullFile.resize(sz);
		input.read(fullFile.data(), sz);
	...
	void CShaderFile::ParseShader(CShader* pShader)
	{
		...
		/*
			I want to go word by word.
			For each word I will first try to match a macro flag, e.g. #if, if that doesn't work I will try to match a stored macro.
				ProcessMacros(std::string s);
		*/
	...
	{
		if(!m_detectComment.InComment(ch, m_contents))
		{
	...
	bool CShaderFile::ProcessMacroDefine(const std::vector<std::string>& wordList)
	{
		...
		/*
			I need to support defined methods.
				The first word is the method name.
				I might have a separate map for defined methods.

				How does replacement work?
					You find a word that matches the method name.
					Collect what is in the preceeding parentheses.
