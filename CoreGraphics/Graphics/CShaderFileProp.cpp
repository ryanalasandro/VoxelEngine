...
namespace Graphics
{
	...
	/*static u32 SplitString(const std::string& str, char divider, std::function<void(const std::string&, u32)> func) {
		u32 index = 0;

		std::string substr = "";
		for(size_t i = 0; i <= str.size(); ++i) {
			if(i == str.size() || str[i] == divider) {
				func(substr, index++);
				substr.clear();
			} else if(str[i] > 0x20) {
				substr += str[i];
	...
	bool CShaderFileProp::ProcessProperty(const Util::CompilerTuple<2>& prop, CShader* pShader)
	{
