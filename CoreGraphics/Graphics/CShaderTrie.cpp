...
namespace Graphics
{
	...
	void CShaderTrie::LoadCSH()
	{
		...
		std::ifstream file(path + filename + L".csh", std::ios::in | std::ios::ate);
	...
	void CShaderTrie::BuildCSH()
	{
		...
		std::ofstream file(path + filename + L".csh");
		ASSERT_R(file.is_open());
	...
	void CShaderTrie::BuildCSHAppend(std::ofstream& file, Node* pNode)
	{
		...
		if(pNode->pShader)
		{
			...
			for(const auto& elem : pNode->macroList)
			{
				...
				if(sz > 0)
				{
					file.write(elem.c_str(), sizeof(char) * sz);
				}
			}
		
			sz = pNode->pShader->GetProperies().size();
			file.write(reinterpret_cast<const char*>(&sz), sizeof(sz));
			file.write(pNode->pShader->GetProperies().c_str(), sizeof(char) * sz);
		
			sz = pNode->pShader->GetHLSL().size();
			file.write(reinterpret_cast<const char*>(&sz), sizeof(sz));
			file.write(pNode->pShader->GetHLSL().c_str(), sizeof(char) * sz);
		}
