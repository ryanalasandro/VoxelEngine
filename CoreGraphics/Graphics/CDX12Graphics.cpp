...
namespace Graphics
{
	...
	void CDX12Graphics::Initialize()
	{
		...
		{ // Describe and create the swap chain.
			...
			{ // Create frame resources.
				...
				ASSERT_HR_R(m_pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_BUNDLE, IID_PPV_ARGS(&m_pBundleAllocator)));
	...
	void CDX12Graphics::Render()
	{
		{ // Populate the command lists.
			...
			ASSERT_HR_R(m_pComputeCommandList->Reset(m_pComputeCommandAllocatorList[m_frameIndex], nullptr/*m_pComputeState*/));
			ASSERT_HR_R(m_pCommandRealtimeList->Reset(m_pCommandAllocatorList[m_frameIndex], nullptr/*m_pPipelineState*/));
	...
	ID3D12GraphicsCommandList* CDX12Graphics::CreateBundle(const CMaterial* pMaterial)
	{
		...
		ASSERT_HR_R(m_pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_BUNDLE, m_pBundleAllocator, pShader->GetPipelineState(), IID_PPV_ARGS(&pBundle)));
