...
namespace Graphics
{
	CPrimQuad::CPrimQuad(const wchar_t* pName, u32 sceneHash) :
	...
	void CPrimQuad::PreRender()
	{
		...
		m_pMaterial->SetFloat(maxtrixBufferHash, wvHash, mtx.f32, 16);
		m_pMaterial->SetFloat(maxtrixBufferHash, projHash, proj.f32, 16);
	}
