...
namespace App
{
	...
	void CPlatform::Quit(int exitCode)
	{
	...
	void CPlatform::SaveProductionScripts()
	{
		...
		size_t sz = m_wuiScript.GetScriptObject().GetCode().size();
		file.write(reinterpret_cast<const char*>(&sz), sizeof(sz));
		file.write(m_wuiScript.GetScriptObject().GetCode().c_str(), sizeof(char) * sz);
	...
	{
		CSceneManager::Instance().OnMove(pPanel);
	}

	void CPlatform::OnResize(const CPanel* pPanel)
	{
		m_pGraphicsAPI->AdjustToResize();
		CSceneManager::Instance().OnResize(pPanel);
